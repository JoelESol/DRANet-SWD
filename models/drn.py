...
def drn26(pretrained=True, finetune=True, out_map=True, model_path="pretrained/drn_c_26-ddedf421.pth", **kwargs):
    model = DRN(BasicBlock, [1, 1, 2, 2, 2, 2, 1, 1], modelname='drn26',
                out_map=out_map, finetune=finetune, **kwargs)

    if pretrained:
        if os.path.exists(model_path):  # Check if local model exists
        ...
        if finetune:
            state_dict.pop('fc.weight', None)  # Use .pop() to avoid KeyError
...
@register_model('drn_pretrain_gta')
def drn_pretrain_gta(pretrained=True, finetune=True, out_map=True, **kwargs):
    model = DRN(BasicBlock, [1, 1, 2, 2, 2, 2, 1, 1], modelname='drn26',
                out_map=out_map, finetune=finetune, **kwargs)
    if pretrained:
        pretrained_path = './pretrained/drn26_iter1700.pth'
        state_dict = torch.load(pretrained_path)  # Correctly loads the state_dict
        model.load_state_dict(state_dict, strict=False)  # Pass it directly
    return model
